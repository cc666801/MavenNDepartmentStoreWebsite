...
public interface OrderRepository extends JpaRepository<Order, Integer> {
	@Query("SELECT o FROM Order o JOIN o.orderStatus os WHERE o.member.id = :memberId AND os.orderStatusName = '貨到付款訂單'")
	List<Order> findAllCashOnDeliverByMemberId(@Param("memberId") Integer memberId);
	
	@Query("SELECT o FROM Order o JOIN o.orderStatus os WHERE o.member.id = :memberId AND os.orderStatusName = '信用卡付款訂單'")
